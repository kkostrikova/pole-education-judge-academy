-- Pole Education · Certificate name confirmation
...
begin
  ...
  select c.certificate_name,c.confirmed_at
  into v_name,v_confirmed_at
  from public.certificate_name_confirmations c
  where c.user_id=v_uid;

  select true,ic.holder_name
  into v_issued,v_issued_name
...
create or replace function public.pe_confirm_certificate_name(p_certificate_name text)
returns jsonb
language plpgsql
security definer
set search_path=public
...
begin
  ...
  insert into public.certificate_name_confirmations(user_id,certificate_name,confirmed_at,updated_at)
...
grant execute on function public.pe_confirm_certificate_name(text) to authenticated;

-- Require confirmed certificate name before issuing a new certificate.
...
begin
  ...
  select certificate_name into v_holder
