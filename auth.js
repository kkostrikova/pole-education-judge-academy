(() => {
  ...
  googleBtn?.addEventListener('click',async()=>{googleBtn.disabled=true;setMsg('Відкриваємо Google…');const redirectTo=new URL(safeNext,location.href).href;const {error}=await client.auth.signInWithOAuth({provider:'google',options:{redirectTo,queryParams:{prompt:'select_account'}}});if(error){googleBtn.disabled=false;setMsg('Не вдалося увійти через Google: '+error.message,'err');}});
  login?.addEventListener('submit',async e=>{e.preventDefault();setMsg('Входимо…');const {error}=await client.auth.signInWithPassword({email:loginEmail.value.trim(),password:loginPassword.value});if(error)return setMsg(error.message,'err');location.href=safeNext;});
  signup?.addEventListener('submit',async e=>{e.preventDefault();setMsg('Створюємо акаунт…');const {data,error}=await client.auth.signUp({email:signupEmail.value.trim(),password:signupPassword.value,options:{data:{full_name:signupName.value.trim(),country:signupCountry.value.trim()},emailRedirectTo:new URL(safeNext,location.href).href}});if(error)return setMsg(error.message,'err');if(data.session){setMsg('Акаунт створено. Переходимо до кабінету…','ok');setTimeout(()=>location.href=safeNext,500)}else setMsg('Готово. Перевірте пошту та підтвердьте email. Після підтвердження поверніться на платформу й увійдіть.','ok');});
  client.auth.getSession().then(({data})=>{if(data.session) location.href=safeNext;});
})();